			...
			try {
				...
				final String unformattedExceptionMessage =
						"No companion class was found for the annotation source class (%1$s)." +
								"Check the following:\n" +
								"- Does the annotation source class have any Spyglass handler annotations?\n" +
								"- Was the Spyglass annotation processor enabled at compile time?\n" +
								"- Were the files generated by the processor deleted manually or by a trimming tool?";

				final String formattedExceptionMessage = String.format(
						unformattedExceptionMessage,
						annotationSource.getCanonicalName());

				throw new InvalidBuilderStateException(formattedExceptionMessage);
			}
